"""
Genera alarmas de Dead Inventory y Mala Exhibición para una fecha objetivo.

Réplica de data-preparation/generate-dead-inventory.js pero leyendo de
...
def load_settings(cur):
    """Lee las keys history_window, dead_inventory y max_alarm_days de la tabla settings."""
    cur.execute("SELECT key, value FROM settings WHERE key IN ('history_window', 'dead_inventory', 'max_alarm_days')")
    rows = cur.fetchall()
    settings = {}
...
def generate_alarms(cur, target_date):
    """Genera alarmas para la fecha objetivo y las inserta en la tabla alarms."""

    # 1. Leer settings
    settings = load_settings(cur)
    if "history_window" not in settings or "dead_inventory" not in settings:
        print("ERROR: Faltan settings (history_window o dead_inventory).")
        return 0
    ...
    # 2. Rango de fechas (usa max_alarm_days para traer datos suficientes para el conteo de alarmas)
    ...
    # Items already sorted by date from ORDER BY
    ...
    # effective_history = min(available_days, max_days), capped
    # Collect all unique dates to determine effective history
    ...
    # Quintile calculation: use dates in [target - effective_history, target) only cataloged
    history_start_idx = target_idx - effective_history
    history_dates = set(all_dates[history_start_idx:target_idx])

    product_sales = defaultdict(float)  # upc -> total
    store_sales = defaultdict(float)    # store_id -> total
    ...
    for (product_id, store_id), items in groups.items():
        ...
        # Skip if inv_on_hand is unknown (day not cataloged properly)
        ...
        ref_id = f"{target_date}_{product_id}_{store_id}"
    ...
    print(f"\nAlarmas generadas: {len(alarms)}")

    if not alarms:
        return 0

    # 7. Insert: delete existing for this date first (idempotency)
    like_pattern = f"{target_date}_%"
    cur.execute("DELETE FROM alarms WHERE ref_id LIKE %s", (like_pattern,))
    deleted = cur.rowcount
    if deleted:
        print(f"Alarmas previas eliminadas: {deleted}")
    ...
    # Count by type
    dead_count = sum(1 for a in alarms if a[2] == "dead_inventory")
    poor_count = sum(1 for a in alarms if a[2] == "poor_display")
    print(f"  dead_inventory: {dead_count}")
    print(f"  poor_display: {poor_count}")
...
def seed_settings(cur):
    """Inserta la configuración necesaria si no existe."""
    cur.execute("SELECT COUNT(*) FROM settings WHERE key IN ('history_window', 'dead_inventory', 'max_alarm_days')")
    count = cur.fetchone()[0]
    if count == 3:
        return

    print("Sembrando settings...")
    cur.execute("""
        INSERT INTO settings (key, value) VALUES
            ('history_window', %s::jsonb),
            ('dead_inventory', %s::jsonb),
            ('max_alarm_days', %s::jsonb)
        ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
    """, (
        json.dumps({"min_days": 14, "max_days": 28}),
...
def main():
    ...
    try:
        ...
        # Generate alarms
        total = generate_alarms(cur, target_date)
        conn.commit()
